...
void SocketRpcChannel::Send(const RpcMessage& message)
{
    ...
    size_t messageSize = proto_buf_remote__rpc_message__get_packed_size(&message);
		...
		while (isReceiveReady && !isTerminated)
		{
			...
			else
			{
				...
				if (receivePos >= receiveSize)
				{
					if (isReceiving)
					{
                        RpcMessage* message = proto_buf_remote__rpc_message__unpack(NULL, receiveSize, 
                            reinterpret_cast<uint8_t*>(&receiveBuffer[0]));
						assert(message);
						m_controller->Receive(*message);
                        proto_buf_remote__rpc_message__free_unpacked(message, NULL);
